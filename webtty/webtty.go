...
func New(masterConn io.ReadWriteCloser, slave Slave, options ...Option) (*WebTTY, error) {
	wt := &WebTTY{
	...
	go func() {
		...
		errs <- func() error {
			...
			for {
				...
				err = wt.handleConnectionReadEvent(buffer[:n])
				if err != nil {
					return err
				}
			}
...
func (wt *WebTTY) connectionWrite(data []byte) error {
	...
	wrote := 0
	for {
		n, err := wt.masterConn.Write(data)
		if err != nil {
